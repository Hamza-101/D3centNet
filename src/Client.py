    ...
    with open(Directories["YellowBook"], "r") as file:
        ...
        for ip, directory_info in transfer_meta.items():
            if filename in directory_info:
                chunks_info = directory_info[filename]["available_chunks"]
                relevant_devices[ip] = {
                    "chunks": chunks_info
                }
...
def receive_chunks(client_socket, chunk_indices):
    ...
    while not received_chunks.issubset(chunk_indices):
        ...
        chunk_number = int(chunk_data.split(b'_')[1].decode())
        output_path = os.path.join(Directories["Path"], f"chunk_{chunk_number}_of_{len(chunk_indices)}")
